...
// const numberOfEvents: number = 100;
// const offerset: Array<string> = ["A", "B"];
...
//   const uniqueOccurrences = Array.from(countMap, ([value, count]) => ({
//     value: offerset[value - 1],
//     count,
//   }));
...
type Statistics = {
  COUNTS: { value: string; percentage: string; count: number }[];
  SAMPLE_SIZE: number;
};

const numberOfEvents: number = 10000; // Adjusted to 10000 for a larger sample
const offerset: Array<string> = ["A", "B"];
const simulatedOccurrences: Array<number> = [];
const statistics: Statistics = { COUNTS: [], SAMPLE_SIZE: numberOfEvents };
...
function countUniqueOccurrences(arr: Array<number>) {
  ...
  const uniqueOccurrences = Array.from(countMap, ([value, count]) => {
    const percentage = ((count / numberOfEvents) * 100).toFixed(2) + "%";
    return { value: offerset[value - 1], percentage, count };
  });
